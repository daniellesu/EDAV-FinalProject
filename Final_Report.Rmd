...
library(extracat)
survey_data <- read_csv("Final_DSI_Playlist.csv", col_names = TRUE)
...
We did quite a bit of analysis using artist's genre so we took a look at how many artists have this information available. As we see below, only 5.3%% of artists in the dataset have missing genre information, or 126 out of 2400 rows in the dataset.
...
```{r loud_box, fig.height = 6, warning=FALSE}
...
```{r tempo_box, fig.height = 6, warning=FALSE}
...
```{r arprep, include = FALSE, warning=FALSE}
...
```{r pop_dot_plot, fig.height = 8, fig.width = 10, warning=FALSE}
